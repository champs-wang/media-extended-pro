...
/**
 * @see https://github.com/vercel/next.js/issues/55648
 */
export const config = {
  matcher: [
...
function locales(request: NextRequest) {
  ...
  const shouldHandleLocale =
    ...
    !/\.(jpe?g|svg|png|webmanifest|xml|ico|txt|gif|mp4)$/.test(
      nextUrl.pathname
    ) &&
    nextUrl.locale !== "" &&
  ...
  if (!pathname.endsWith("." + finalLocale)) {
    ...
    return NextResponse.rewrite(new URL(url, request.url), { status: 200 });
  }
}
